...
const appendSideArrows = (container: HTMLDivElement) => {
  [Classes.previousImage, Classes.nextImage].forEach((klass) => {
    const currentArrow =
      klass === Classes.previousImage
        ? DataClick.arrowPrevious
        : DataClick.arrowNext;
...
const createDotsContainer = ({ parent }: DotsProps): CarouselDots => {
  ...
  const _getDotByIndex = (index: number): Dot => {
    // returns null if dot does not exists
    return dotsContainer.querySelector(
      `.${Classes.dot}[data-index="${index}"]`,
    );
  };
...
const appendImageFrames = (
  container: HTMLDivElement,
  images: Image[],
  Dots: CarouselDots,
  activeFrameIx: FrameIndexRef,
) => {
  let index = 0;
